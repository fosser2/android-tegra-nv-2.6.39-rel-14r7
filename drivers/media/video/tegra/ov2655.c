 ...
 * Please add /dev/ov2655 0660 media camera in below file
...
#define DEFAULT_SVGA_DUMMY_LINE  0x026a
#define DEFAULT_UXGA_DUMMY_LINE  0x04d4

#define DEFAULT_SVGA_MAX_SHUTTER  0x01
#define DEFAULT_UXGA_MAX_SHUTTER  0x02

#define PREVIEW_PCLK_FREQUENCY  2400
#define CAPTURE_PCLK_FREQUENCY  3200

#define CAPTURE_MAX_GAIN16   8
#define OV2655_SENSOR_DEBUG 1
#if (OV2655_SENSOR_DEBUG)
#define logd(x...)	do { pr_info(x); } while(0)
#else
#define logd(x...)	do { } while(0)
#endif

/* SVGA: 800 * 600, UXGA: 1600*1200 */
...
static struct sensor_reg mode_640x480_old[] = {
{SENSOR_WAIT_MS, 1},
{0x3012, 0x80},
/* Add some dealy or wait a few miliseconds
after register reset & Clock & Analog Setup */
{0x308c, 0x80},
{0x308d, 0x0e},
{0x360b, 0x00},
{0x30b0, 0xff},
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case OV2655_IOCTL_GET_STATUS:
	{
		return 0;
	}
	case OV2655_IOCTL_SET_COLOR_EFFECT:
	{
		u8 coloreffect;
		...
		logd("%s,IOCTL_SET_COLOR_EFFECT,coloreffect is %d\n",__func__, coloreffect);
		switch(coloreffect) {
	...
	case OV2655_IOCTL_SET_WHITE_BALANCE:
	{
		u8 whitebalance;
		...
		logd("%s,IOCTL_SET_WHITE_BALANCE,whitebalance is %d\n",__func__, whitebalance);
		switch(whitebalance) {
	...
	case OV2655_IOCTL_SET_SCENE_MODE:
		return 0;
	case OV2655_IOCTL_SET_EXPOSURE:
	{
		u8 exposure;
		...
		logd("%s, IOCTL_SET_EXPOSURE,exposure is %d\n",__func__, exposure);
		switch(exposure) {
		case YUV_Exposure_0:
